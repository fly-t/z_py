...
class LocalMarketNotify(EmailNotify):
    ...
    Market days default to lunar days {2, 7,11, 12, 17, 22, 27}.
    """
    ...
    def check_market_day(self, on_date: date = None) -> bool:
        ...
        if lunar_day in self.market_days:
            ...
            self.send_email(to_addr="1312765847@qq.com",send_name="ql通知", msg_subject="赶集日通知",  msg_content=f"今天是农历 {lunar.month}月{lunar.day}日，是赶集日")
            self.send_email(to_addr="3020909671@qq.com",send_name="ql通知", msg_subject="赶集日通知",  msg_content=f"今天是农历 {lunar.month}月{lunar.day}日，是赶集日")
            return True
        else:
